...
main(List<String> args) {
  ...
  if (args.length == 0) {
    ...
    print('Usage: extract_to_json [options] [files.dart]');
    print(parser.usage);
    exit(0);
  }
  var allMessages = [];
...
Map toJSON(MainMessage message) {
  ...
  var out = {};
  out["term"] = message.name;

  print('\n=== ${message.name}');
  message.messagePieces.forEach((dynamic chunk) {
    if (chunk is String) {
     print('  - string: ${getValue(chunk)}');

    } else if (chunk is int && chunk >= 0 && chunk < message.arguments.length) {
      print("  - int: ${message.arguments[chunk]}");

    } else if (chunk is SubMessage) {
      print('  - SubMessage:');

      (chunk as SubMessage).attributeNames.forEach((attr) {
        print('    ~ attr: $attr = ${getValue(chunk[attr])}');
      });
  ...
  out["definition"] = icuForm(message);
  out["comment"] = message.description;
  return out;
}
...
String getValue(message) {
  if (message is CompositeMessage) {
    return (message as CompositeMessage).pieces.map(getValue).join('');
  }
  if (message is LiteralString) {
    return (message as LiteralString).string;
  }
  if (message is VariableSubstitution) {
    return '{${(message as VariableSubstitution).name}}';
  }
